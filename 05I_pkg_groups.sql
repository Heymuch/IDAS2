...
CREATE OR REPLACE PACKAGE BODY PKG_GROUP AS
    ...
    BEGIN
        OPEN v_cursor FOR
            SELECT VW_GROUPS.GROUP_ID, VW_GROUPS.TEACHER_ID, TEACHER_NAME, GROUP_NAME, MAX_CAPACITY, ACTUAL_CAPACITY, FREE_CAPACITY FROM VW_GROUPS
                LEFT JOIN TEACHERS ON TEACHERS.TEACHER_ID = VW_GROUPS.TEACHER_ID
                LEFT JOIN STUDENTS_GROUPS ON STUDENTS_GROUPS.GROUP_ID = VW_GROUPS.GROUP_ID
                LEFT JOIN STUDENTS ON STUDENTS_GROUPS.STUDENT_ID = STUDENTS.STUDENT_ID
            WHERE TEACHERS.USER_ID = p_user_id OR STUDENTS.USER_ID = p_user_id
            GROUP BY GROUP_NAME, VW_GROUPS.TEACHER_ID, TEACHER_NAME, VW_GROUPS.GROUP_ID, MAX_CAPACITY, ACTUAL_CAPACITY, FREE_CAPACITY;
        END;
